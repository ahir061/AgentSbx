...
import agentsbx
from agentsbx import Sandbox, ShellResult

# Private helpers aren't re-exported from the package root, so they come from
# the module directly.
from agentsbx.sandbox import _truncate
...
def test_public_api_is_importable_from_the_package_root():
    # `from agentsbx import Sandbox` is the documented entry point;
    ...
    for name in agentsbx.__all__:
        assert hasattr(agentsbx, name), f"{name} is in __all__ but not exported"
